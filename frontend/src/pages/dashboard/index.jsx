...
export default function Dashboard() {
  ...
  const handleUpload = async()=>{ 
    if (!fileContent && !postContent) {
      alert("Please provide content and file!");
      return;
    }
...
const handleLike = async (postId) => {
  const response = await dispatch(incrementLike({ post_id: postId }));
  await dispatch(getAllPosts());

  const message = response?.payload?.message; // adjust depending on how your action returns data
          ...
          <DashboardLayout>
              <div className={styles.scrollComponent}>
                ...
                <div className={styles.wrapper}>
                  <div className={styles.createPostSearchContainer}>
                      ...
                      <img className={styles.userProfile}
                          ...
                          src={
                            authState?.user?.userId?.profilePicture
                              ...
                              : "https://img.freepik.com/free-vector/blue-circle-with-white-user_78370-4707.jpg?semt=ais_hybrid&w=740" // use a default image if undefined
                          }
  ...
  const isLikedByMe = post.likedBy.includes(currentUserId); // Adjust field name if needed
                        return (
                          <div key={post._id} className={styles.singleCard}>
                            <div className={styles.singleCardProfileContainer}>
                               <img onClick={() => router.push(`/view_profile/${post.userId.username}`)} className={styles.userProfile} src={`${BASE_URL}/${post.userId.profilePicture}`} alt="" />

                              <div>
                              
                              <div style={{display:"flex", gap:"1.2rem", justifyContent:"space-between"}}>
                                <div className={styles.daysAgoContainer}>

                              <p onClick={() => router.push(`/view_profile/${post.userId.username}`)} style={{fontWeight:"bold",cursor:"pointer"}}>{post.userId.name}</p>
                              <p style={{fontWeight:"lighter"}}>{dayjs(post.createdAt).fromNow()}</p>
                                </div>
                              ...
                              <p onClick={() => router.push(`/view_profile/${post.userId.username}`)} style={{color: "grey",cursor:"pointer"}}>@{post.userId.username}</p>
...
>
  <svg
    ...
    strokeWidth="1.5"
    stroke="currentColor"
    className={`${styles.size6} ${styles.likeIcon} ${likedPosts.has(post._id) ? styles.liked : ''}`}
  >
    <path
      strokeLinecap="round"
...
<p>{post.likes}</p>
                              ...
                              {/* <p>{post.comment.length}</p> */}
                                ...
                                <div onClick={() =>{
                                  const text  = encodeURI(post.body);
                                  const url = encodeURIComponent("linkedin.com");
                                  const twitterUrl = `https://twitter.com/intent/tweet?text=${text}&rul=${url}`
              ...
              {postState.postId !== "" && (
  <div onClick={() => dispatch(resetPostId())} className={styles.overlay}>
    <div onClick={(e) => e.stopPropagation()} className={styles.modal}>
      <div className={styles.header}>
        <h2>Comments</h2>
        <button className={styles.closeBtn} onClick={() => dispatch(resetPostId())}>✕</button>
      </div>
